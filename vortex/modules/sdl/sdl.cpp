...
extern "C" Value initSDL(std::vector<Value> &args)
{
    ...
    int init = SDL_Init(SDL_INIT_EVERYTHING & ~(SDL_INIT_TIMER | SDL_INIT_HAPTIC));
...
extern "C" Value wasm_main_loop(std::vector<Value> &args)
{
    int num_required_args = 2;

    if (args.size() != num_required_args)
    {
        return error_object("Function 'wasm_main_loop' expects " + std::to_string(num_required_args) + " argument(s)");
    }
    ...
    if (!func.is_function())
    {
        return error_object("Function 'wasm_main_loop' expects arg 'func' to be a function");
    }

    if (!loop.is_boolean())
    {
        return error_object("Function 'wasm_main_loop' expects arg 'simulate_infinite_loop' to be a function");
    }

    auto func_ptr = std::make_shared<Value>(func);

    emscripten_set_main_loop_arg(mainLoopWrapper, &func_ptr, 0, (int)loop.get_boolean());
